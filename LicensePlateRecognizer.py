# -*- coding: utf-8 -*-
"""
Created on Sun Jul  4 23:29:53 2021
...
def startanalyzing():
    ...
    net = cv2.dnn.readNet("yolov4-obj_6000.weights" , "yolov4-custom.cfg")
    net.setPreferableBackend(cv2.dnn.DNN_BACKEND_CUDA)
    net.setPreferableTarget(cv2.dnn.DNN_TARGET_CUDA)
    
    char_net = cv2.dnn.readNet("yolov4-obj_50000_char_final.weights" , "yolov4-obj_char_final.cfg")
    ...
    if(video == 1):
        ...
        while True:
            ...
            if cv2.waitKey(1)==27:
                break
            startmodel(img,net,char_net,classes,char_classes,num_class,layer_names,output_layers,colors,char_layer_names,char_output_layers,char_colors)
...
def startmodel(img,net,char_net,classes,char_classes,num_class,layer_names,output_layers,colors,char_layer_names,char_output_layers,char_colors):
    
    start_time = time.clock()
    
    #img = imutils.resize(img, width=416, height=416, inter=cv2.INTER_CUBIC)
    ...
    for i in range(len(boxes)):
      if i in indexes:
          ...
          char_img = new_img
          #char_img = cv2.resize(char_img, None , fx=2, fy=2)
          
          #char_img=cv2.detailEnhance(char_img, 10, 0.99)
          
          char_height, char_width, char_channels = char_img.shape
        
          #detecting objects
          char_blob = cv2.dnn.blobFromImage(char_img, 0.00392, (256,256), (0,0,0), True, crop = False)
